...
class SIMPLEPreProcessor01():
    ...
    __optional_params = ['csv_1','csv_2','csv_3']
    #__optional_params = ['/home/ubuntu/simple_allcrops/01_data_clean/in/reg_sets.csv','/home/ubuntu/simple_allcrops/01_data_clean/in/crop_sets.csv','/home/ubuntu/simple_allcrops/01_data_clean/in/livestock_sets.csv']
    __required_params = ['start_year','end_year','in_path','out_path']
    ...
    def __init__(self, **kwargs):
        ...
        for param in self.__required_params:
            if param not in kwargs:
               raise RuntimeError('Please enter parameters!')
        ...
        if len(self.l_val) == 4:
            self.opt_val = ['/home/ubuntu/simple_allcrops/01_data_clean/in/reg_sets.csv','/home/ubuntu/simple_allcrops/01_data_clean/in/crop_sets.csv','/home/ubuntu/simple_allcrops/01_data_clean/in/livestock_sets.csv']
        else:
            self.opt_val = []
            self.opt_val.append(self.l_val[4])
            self.opt_val.append(self.l_val[5])
            self.opt_val.append(self.l_val[6])

        if int(float(self.l_val[0])) > int(float(self.l_val[1])):
            raise RuntimeError('Start year must be smaller than or equal to end year!')
    ...
    def process(self):
        ...
        try:
            ...
            command = "Rscript"
            path2script = "/home/ubuntu/simple_allcrops/01_data_clean/01_data_clean.r"

            cmd = [command,path2script] + self.l_val + self.opt_val
            
            x = subprocess.check_output(cmd,universal_newlines=True)

            print("R file running is complete!")
           ...
           # shapefileReprojector.target_path = self.target_path
            ...
            print('Error in running data cleaning R file')
